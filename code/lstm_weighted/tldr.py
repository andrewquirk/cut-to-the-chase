...
class Encoder(object):
    def __init__(self,FLAGS):
        ...
        self.cell = tf.nn.rnn_cell.BasicLSTMCell(self.state_size, state_is_tuple = False)
        self.cell2 = tf.nn.rnn_cell.BasicLSTMCell(self.state_size, state_is_tuple = False)
        with vs.variable_scope("encoder", initializer = tf.contrib.layers.xavier_initializer()):
            ...
            # self.b = tf.get_variable("b", dtype = tf.float64, shape = (2*self.state_size)
            self.W2 = tf.get_variable("W2", dtype = tf.float64, shape = (self.state_size, self.state_size))
            self.b2 = tf.get_variable("b2", dtype = tf.float64, shape = (self.state_size))    
    ...
    def encode_sentences(self, article, seqlen):
        ...
        o, h_sentences = tf.nn.dynamic_rnn(self.cell, article, sequence_length = seqlen, dtype = tf.float64)
        # print(o)
        # o = o[:self.state_size]
        h_sentences = h_sentences[:, :self.state_size]
        # o is numsentences by maxSentenceLength by stateSize
        # h is numsentences by stateSize
        return o, h_sentences
    def inter_relu(self, h_sentences):
        m = tf.nn.relu(tf.matmul(h_sentences, self.W2) + self.b2)
        h_expanded = tf.expand_dims(m, axis = 0)
        return h_expanded
    def encode_article(self, article, h_expanded):
        ...
        with vs.variable_scope("article"):
            ...
            # print(h_article)
            h_article = h_article[:,:self.state_size]
            # print(h_article)
...
class TldrSystem(object):
    ...
    def setup_system(self):
        ...
        h_sentences_drop = tf.nn.dropout(h_sentences, self.drop_placeholder)
        h_expanded = self.encoder.inter_relu(h_sentences_drop)
        h_article = self.encoder.encode_article(self.x, h_expanded)
        ...
        self.pred = self.classifier.classify(h_drop)
    ...
    def setup_loss(self):
        with vs.variable_scope("loss"):
            ...
            l = tf.nn.weighted_cross_entropy_with_logits(logits = self.pred, 
            targets = tf.expand_dims(self.yplaceholder, 1), pos_weight = 3.0)
            self.loss = tf.reduce_sum(l)
            # self.loss = 0.0
            # for i in range(len(self.pred)):
                # y = self.yplaceholder[i]
                # if y==1:
                    # self.loss += 
            # maybe switch back to sum later

    def optimize(self, sess, train_x, train_y, train_mask, drop, lr):
        input_feed = {}
    ...
    def test(self, sess, dev_x, dev_y, dev_mask, drop=1.0, lr = 0):
        input_feed = {}
    ...
    def answer(self, sess, dev_x, dev_y, dev_mask, drop=1.0, lr = 0):
        input_feed = {}
        input_feed[self.xplaceholder] = dev_x
        input_feed[self.yplaceholder] = dev_y
        input_feed[self.maskplaceholder] = dev_mask
    ...
    def train(self, sess, vocab, rev_vocab):
        ...
        num = int(num)
        thresh = 2
        coeff = num // thresh + num % thresh
        dataSize = self.FLAGS.train_size / 4 
        # print(num)
        for e in range(num+1, self.FLAGS.numEpochs, 1):
            ...
            i = 0
            average_time = 0
            current_lr = self.FLAGS.learning_rate # not annealing for now
            # print(coeff)
            for data in self.readPreprocessedData(self.FLAGS.train_directory):
                i += 1
                # if i==1: break # to stop training and just test
                ...
                sentMask, 1.0 - self.FLAGS.dropProb, current_lr) 
                batch_time = time.time() - start
                average_time = average_time * (i-1) / i + batch_time * 1/i
                eta = (dataSize - i) * average_time
                # print(coeff)
